...
import { computed, nextTick, onMounted, onUpdated, ref, toRefs } from 'vue'
import type { ContentType, ContentPosition } from './type.ts'
...
async function init() {
  ...
  allData.value = tmpArr.map<ContentType>((item, idx) => ({ ...item, arrPos: idx }))
...
function updateHeightAndPos() {
  ...
  pillarDomHeight.value =
    positionDataArr.length > 0 ? positionDataArr[positionDataArr.length - 1].endPos : 0
}
...
<template>
  ...
  <div class="scrollerContainer" ref="scrollerContainerRef" @scroll="onScroll">
    ...
    <div class="contentList" :style="styleTranslate" ref="contentListRef">
      <div
        class="item"
        v-for="oneData in renderData"
        :key="oneData.id"
        :data-index="oneData.arrPos"
      >
        <div>{{ oneData.arrPos }}</div>
        <p>{{ oneData.content }}</p>
      </div>
    </div>
  </div>
